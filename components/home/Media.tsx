...
export default function Media() {
      ...
      {carouselItems.map((item) => (
        ...
        >
          ...
          <div className={cx("absolute top-5 left-5 inline-flex items-center gap-2 px-4 py-1.5 rounded-full", item.badgeBg)}>
            {/* Using the custom SVG gradient defined at the top of the section */}
            <item.icon className="w-3.5 h-3.5" style={{ stroke: "url(#icon-gradient)" }} />
  ...
  return (
    <section className="force-trebuchet relative w-full py-24 bg-transparent overflow-hidden z-10">
      ...
      {/* This hidden SVG defines the gradient so Lucide icons can use it via 'stroke="url(#icon-gradient)"' 
        Blue-400 is #60a5fa, Indigo-500 is #6366f1
      */}
      ...
      <div className="relative z-10 flex flex-col items-center text-center mb-16 px-4">
        {/* Updated "Media Transmissions" Header Badge to match the gradient */}
        <div className="flex items-center gap-3 mb-4 rounded-full bg-indigo-500/10 border border-indigo-500/30 px-4 py-1.5">
          <Radio className="h-4 w-4 animate-pulse" style={{ stroke: "url(#icon-gradient)" }} />
          <h2 className="text-[10px] font-bold tracking-[0.25em] uppercase text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-indigo-500">
            Media Transmissions
          </h2>
        </div>
        
        <h3 className="text-4xl sm:text-5xl md:text-6xl font-extralight tracking-tight text-white mb-4">
          Global <span className="font-normal text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-indigo-500">Coverage</span>
        </h3>
      </div>
            ...
            <div className="relative z-10 flex flex-col h-full justify-between p-8 sm:p-12">
              <div className="flex flex-wrap items-center gap-3">
                <div className={cx("inline-flex items-center gap-2 px-4 py-2 rounded-full border backdrop-blur-md", heroItem.badgeBg, heroItem.badgeBorder)}>
                  {/* Using the custom SVG gradient */}
                  <heroItem.icon className="w-4 h-4" style={{ stroke: "url(#icon-gradient)" }} />
